...
public class FButton : FContainer, FSingleTouchableInterface
{
	...
	public bool ButtonEnabled { get { return buttonEnabled; } set { buttonEnabled = value;  SetDisableColor();} }
	...
	{
		Debug.Log("HandleSingleTouchEnded");
		_isTouchDown = false;
		...
		if(expandedRect.Contains(touchPos))
		{
			...
			if(_supportsOver && _hitRect.Contains(touchPos)) //go back to the over image if we're over the button
			{
				...
				Debug.Log("HandleSingleTouchEnded _isEnabled = true");
				_isEnabled = true;
	...
	virtual public void HandleSingleTouchCanceled(FTouch touch)
	{
		...
		Debug.Log("HandleSingleTouchCanceled");
		_isTouchDown = false;
